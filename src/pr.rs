...
impl ProactiveRefresh {
    ...
    pub fn refresh_all(&mut self) {
        ...
        for (j, key) in self.tkp.keys.iter().enumerate() {
            updated_keys.push(self.update_1(key.priv_key(), j));
        } 
        for (j, key) in self.tkp.keys.iter_mut().enumerate() {
            key.update_secret(updated_keys[j]);
        } 
    }
